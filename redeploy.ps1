...
function Build-Connected {
    ...
    docker build --build-arg CACHE_GRYPE_DB=false -t ghcr.io/jdmldm1/periscope-kubernetes:1.1.2-connected .

    Write-Host "Creating Connected Zarf package..."
    if (Test-Path .\zarf-package-periscope-amd64-1.1.2.tar.zst) {
        Remove-Item .\zarf-package-periscope-amd64-1.1.2.tar.zst -Force
    }
...
function Build-Airgap {
    ...
    docker build --build-arg CACHE_GRYPE_DB=true -t ghcr.io/jdmldm1/periscope-kubernetes:1.1.2 .

    Write-Host "Creating Airgap Zarf package..."
    if (Test-Path .\zarf-package-periscope-airgap-amd64-1.1.2.tar.zst) {
        Remove-Item .\zarf-package-periscope-airgap-amd64-1.1.2.tar.zst -Force
    }
...
if ($Mode -eq "Connected") {
    ...
    zarf package deploy .\zarf-package-periscope-amd64-1.1.2.tar.zst --confirm
...
elseif ($Mode -eq "Airgap") {
    ...
    zarf package deploy .\zarf-package-periscope-airgap-amd64-1.1.2.tar.zst --confirm
